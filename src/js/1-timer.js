...
const inputOptions = {
  ...
  onClose(selectedDates) {
    // Обробник події, що викликається при закритті вікна вибору дати
    targetDate = selectedDates[0];

    // Перевірка на коректність дати
    const isDateValid = Date.now() < targetDate.getTime();
    // Повідомлення про помилку, якщо дата некоректна
    if (!isDateValid) {
      iziToast.show({
        title: 'Error',
        titleColor: '#FFF',
...
// Функція для підрахунку значень часу з мілісекунд
function convertMs(ms) {
  ...
  const day = hour * 24;
   ...
   // Залишок днів, годин, хвилин і секунд
